...
impl Board for Pc3877ms {
    ...
    fn parse_raw_data(board_specific_parameters: &[u8], data: &[u8]) -> anyhow::Result<EventData> {
        if !data.len().is_multiple_of(8) {
            bail!("Pc3877ms Event data is not a multiple of pairs of 4-byte words");
        }
        if board_specific_parameters.len() != 8 {
            bail!("Pc3877ms board-specific parameters should have length 8 bytes")
        }
...
impl TestableBoard for Pc3877ms {
    fn make_fake_event() -> Vec<u8> {
        ...
        // val = 123
        vec![
            0xE0, 0, 89, 16, // E0 tof marker + 20ns (scaling) * (89 * 256 + 16) = 456000ns
            0b11111110, 0xFF, 0, 123, // Position 123
        ]
    }
    ...
    fn make_fake_board_specific_header_data() -> Vec<u8> {
        [
            0x78, // Board address
            7,    // Bits for channel
            16,    // Bits for diagnostic data
            8,    // Bits for position data
        ]
        .into_iter()
...
mod tests {
    ...
    fn test_parse_pc3877_event() {
        ...
        let board_specific_header_data = Pc3877ms::make_fake_board_specific_header_data();

        let events = Pc3877ms::parse_raw_data(&board_specific_header_data, &data)
